...
////            if(!*state) {
               ...
               Composition::Layer *layer = view->getLayerTreeLimb()->getRootLayer();
               auto & style = layer->getStyle();
               style->setBrush(0,Composition::ColorBrush(Composition::Color(0xAA0000)));
               view->getLayerTreeLimb()->redraw();
            //    *state = !*(state);
////            };
...
//            };
        ...
        void onMouseEnter(Native::NativeEventPtr event){
              Composition::Layer *layer = view->getLayerTreeLimb()->getRootLayer();
               auto & style = layer->getStyle();
               style->setBrush(0,Composition::ColorBrush(Composition::Color(0xCC0000)));
               view->getLayerTreeLimb()->redraw();
        };
...
public:
    MyWidget(const Core::Rect & rect,SharedHandle<Media::BitmapImage> & img):Widget(rect,nullptr),delegate(new MyRootDelegate(&selected)){
        ...
        layerStyle->add(VISUAL_RECT(OmegaWTK::Rect(0,0,300,300),ColorBrush(Composition::Color::Red)));
